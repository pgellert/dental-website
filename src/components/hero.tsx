    ...
    <>
      ...
      <div className="md:flex md:flex-wrap lg:flex-nowrap w-full">
        <div className="w-full mt-3 flex shrink flex-wrap items-center">
          ...
          <div className="w-full">{CTAButtons()}</div>
        </div>
      </div>
    </>
  )
...
export function CTAButtons() {
  ...
  const content = data[locale]
  return (
    <div className="place-items-center center mt-10 flex flex-col md:flex-row items-baseline justify-center gap-6 space-y-3 px-4 w-full xl:m-0 xl:flex-row xl:items-center xl:space-x-4 xl:space-y-0">
      <Link
        href="/contact-us"
        className="rounded-md bg-sky-600 px-8 py-4 text-center text-xl font-medium shadow-md text-white w-full md:w-auto"
      >
        {content.booking}
      </Link>
      ...
      <a
        ...
        className="bg-grey-600 rounded-md border-2 border-sky-600 px-8 py-4 text-center text-xl font-medium text-sky-600 w-full md:w-auto"
      >
        {content.call}
      </a>
    </div>
